...
class MemoryEfficientReplay:
    ...
    def feed(self, experience):
        if np.random.rand() < self.drop_prob:
            return
        if len(self.data) == 0:
            # First time, create data structure
            for e in experience:
                if not isinstance(e, np.ndarray):
                    e = np.array(e)
                self.data.append(np.zeros(shape=(self.memory_size, ) + e.shape, dtype=e.dtype))
        self.length = min(self.length + 1, self.memory_size)
        for d, e in zip(self.data, experience):
            d[self.pos] = e
    ...
    def sample(self, batch_size=None):
        ...
        sampled_indices = np.random.randint(0, self.length, size=(batch_size,))
        sampled_data = [d[sampled_indices] for d in self.data]
        if self.to_np:
    ...
    def dump(self, filename):
        with open(filename, 'wb') as f:
            pickle.dump({"data": self.data, "pos": self.pos, "length": self.length}, f, protocol=pickle.HIGHEST_PROTOCOL)
...
class Replay:
    ...
    def sample(self, batch_size=None):
        ...
        # sampled_indices = [np.random.randint(0, len(self.data)) for _ in range(batch_size)]
        sampled_indices = np.random.randint(0, len(self.data), size=(batch_size,))
    ...
    def dump(self, filename):
        with open(filename, 'wb') as f:
            pickle.dump({"data": self.data, "pos": self.pos}, f, protocol=pickle.HIGHEST_PROTOCOL)
