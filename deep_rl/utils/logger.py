...
def get_logger(tag='default', log_level=0, models_path='./'):
    logger = logging.getLogger()
    logger.setLevel(logging.INFO)
    if tag is not None:
        fh = logging.FileHandler('./%s/log/%s-%s.txt' % (models_path, tag, get_time_str()))
        fh.setFormatter(logging.Formatter('%(asctime)s - %(name)s - %(levelname)s: %(message)s'))
        fh.setLevel(logging.INFO)
        logger.addHandler(fh)
    return Logger(logger, './%s/tf_log/logger-%s-%s' % (models_path, tag, get_time_str()), log_level)
